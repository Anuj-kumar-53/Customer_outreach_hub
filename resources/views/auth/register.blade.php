    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        @if (session()->has('referral_referrer_id') && session()->has('referral_campaign_id'))
            <div class="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
                {{ __('You are registering through a referral link. When you finish signing up, your referrer may earn reward points.') }}
            </div>
        @endif
        ...
        <div class="mt-4">
            ...
            <select id="role" name="role" required class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                <option value="customer" @selected(old('role', 'customer') === 'customer')>{{ __('Customer') }}</option>
                <option value="business" @selected(old('role') === 'business')>{{ __('Business') }}</option>
            </select>
        ...
        <div class="flex items-center justify-between mt-4">
            <a class=" text-sm  rounded-md border p-2 bg-black text-white text-center" href="{{ url('/') }}">
